...
public abstract class AspSyntax {
    ...
    /**
     * First checks if the token given is correct, if not then an error is returned
     * If the token is correct, then we can just read the next token. This is useful 
     * for syntax in where their diagram is given such as " [  expr  ]". In 
     * this situation, it isn´t necessary to read the brackets, therefore we can just 
